...
program
  ...
  .option(
    '-a, --all',
    'Run snappydoo for all snapshots, not just modified ones'
  )
  .parse(process.argv)
...
async function main() {
  ...
  snapshotFiles = snapshotFiles.filter(file => {
    return path.extname(file) === '.snap' && program.all
      ? true
      : modifiedFiles.indexOf(file.replace(`${process.cwd()}/`, '')) > -1
  })
  ...
  snapshotFiles.forEach(async file => {
    ...
    const match = new RegExp('(.*)/?__snapshots__/(.*).test.(js|ts).snap').exec(
      file
    )
    if (match) {
  ...
  console.log(
    `Fetching ${Object.keys(snapshots).length} screenshot${
      Object.keys(snapshots).length === 1 ? '' : 's'
  ...
  await browser.close()
  console.log(
    `Snappydoo done in ${(new Date() - start) /
      1000}s. Created ${fileCreationCounter} file${
      fileCreationCounter === 1 ? '' : 's'
    }`
  )
}
